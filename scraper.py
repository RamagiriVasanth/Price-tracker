...
def scrape_price(url):
    ...
    try:
        ...
        response.raise_for_status()  # Raise error for any failed HTTP requests

        soup = BeautifulSoup(response.text, 'html.parser')

        # Scrape price from an Amazon page (adjust based on the actual page structure)
        price_tag = soup.find('span', {'class': 'a-price-whole'})  # Modify based on your target page structure
        if price_tag:
